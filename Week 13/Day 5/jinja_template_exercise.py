...
jetson_list = []
for char in jetsons_details.values():
    jetson_list.append(char)

app = Flask(__name__)

@app.get("/details")
def details():
    return render_template("jetsonabout.html", jetson_list=jetson_list)

@app.get("/character/<name>")
def charater(name):
    for char in jetson_list:
        if name == char["name"]:
            print(char)
            return render_template("jetsonchar.html", char = char)
...
import random
@app.get("/jetsontrivia")
def jetsontrivia():
    randint = random.randrange(0,10)
    question = jetsons_trivia[randint]
    return render_template("jetsonquiz.html", question = question, randint = randint)

@app.post("/jetsonanswer")
def jetsonanswer():
    id = request.json.get("id")
    print(id)
    guess = request.json.get("guess")
    print(guess)
    if jetsons_trivia[id]["answer"] == guess:
